<?php
    ...
    $lockPositionDate = date('Y-m-d H:i:s');
    $code = $_POST['code'];
    $lockPositionType;
    $codeType;
    ...
    $sql = "UPDATE userInfo SET ".$lockPosition." = ?, ".$lockPosition."_regDate"." = ? WHERE nickname = ?";
    $stmt = $conn->prepare($sql);
    if ($stmt === false) {
        die('Prepare failed: ' . $conn->error);
    }

    $stmt->bind_param('sss', $code, $lockPositionDate, $nickname);
